...
func main() {
	...
	topics := []broker.Topic{
		{Topic: broker.CreateTransfer, NumPartitions: int(c.HIGH_PRIORITY_PARTITION), ReplicationFactor: 1, Priority: "HIGH_PRIORITY_PARTITION"},
	}
	...
	// listen to topics
	go broker.Subscribe(client, []string{"update_transaction"}, l)
